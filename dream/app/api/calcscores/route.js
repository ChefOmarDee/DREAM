...
export async function GET(req) {
  try {
    ...
    const startZipcode = '72379';  // Set the starting zipcode

    let cursor = ZipCode.find({ zipcode: { $gte: startZipcode } })
                        .select('-geojson')
                        .sort({ zipcode: 1 })  // Ensure zipcodes are processed in order
                        .cursor();

    console.log(`Starting document processing from zipcode ${startZipcode}...`);
